//
// AUTO-GENERATED FILE, DO NOT MODIFY!
//
// @dart=2.0

// ignore_for_file: unused_element, unused_import
// ignore_for_file: always_put_required_named_parameters_first
// ignore_for_file: constant_identifier_names
// ignore_for_file: lines_longer_than_80_chars

part of polaris;

class GeocodeResults {
  /// Returns a new [GeocodeResults] instance.
  GeocodeResults({
    this.results = const [],
  });

  List<PlaceItem> results;

  @override
  bool operator ==(Object other) => identical(this, other) || other is GeocodeResults &&
     other.results == results;

  @override
  int get hashCode =>
  // ignore: unnecessary_parenthesis
    (results == null ? 0 : results.hashCode);

  @override
  String toString() => 'GeocodeResults[results=$results]';

  Map<String, dynamic> toJson() {
    final json = <String, dynamic>{};
      json[r'results'] = results;
    return json;
  }

  /// Returns a new [GeocodeResults] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static GeocodeResults fromJson(dynamic value) {
    if (value is Map) {
      final json = value.cast<String, dynamic>();
      return GeocodeResults(
        results: PlaceItem.listFromJson(json[r'results']),
      );
    }
    return null;
  }

  static List<GeocodeResults> listFromJson(dynamic json, {bool emptyIsNull, bool growable,}) =>
    json is List && json.isNotEmpty
      ? json.map(GeocodeResults.fromJson).toList(growable: true == growable)
      : true == emptyIsNull ? null : <GeocodeResults>[];

  static Map<String, GeocodeResults> mapFromJson(dynamic json) {
    final map = <String, GeocodeResults>{};
    if (json is Map && json.isNotEmpty) {
      json
        .cast<String, dynamic>()
        .forEach((key, dynamic value) => map[key] = GeocodeResults.fromJson(value));
    }
    return map;
  }

  // maps a json object with a list of GeocodeResults-objects as value to a dart map
  static Map<String, List<GeocodeResults>> mapListFromJson(dynamic json, {bool emptyIsNull, bool growable,}) {
    final map = <String, List<GeocodeResults>>{};
    if (json is Map && json.isNotEmpty) {
      json
        .cast<String, dynamic>()
        .forEach((key, dynamic value) {
          map[key] = GeocodeResults.listFromJson(
            value,
            emptyIsNull: emptyIsNull,
            growable: growable,
          );
        });
    }
    return map;
  }
}

